sim_robustCsi <- function(object, Amat=NULL, bvec=NULL, 
                          meq=0, N=c(15,25,50), d=0, eta.vec=12, cont=0.10, cont.x=1, 
  ...
  for(j in 1:S) {
    ...
    for(i in 1:R) {
      ...
      X <- mvtnorm:::rmvnorm(n, mean=rep(0,4), sigma=diag(4))
      y <- cbind(c(betas[-1]%*%t(X))) + rnorm(n)   
      
      idx <- sample(1:nrow(y), n*cont, replace=FALSE)                             
      X[idx,cont.x] <- rnorm(length(idx), 5, 0.1)  
      y[idx] <- rnorm(length(idx), eta, 0.1)  
      
      object <- MASS:::rlm(y~X, method="MM")
      
      out <- csi.rlm(object, Amat, bvec, meq)
